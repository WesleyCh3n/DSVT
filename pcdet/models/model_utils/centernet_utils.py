...
def decode_bbox_from_pred_dicts(
    pred_dict, point_cloud_range=None, voxel_size=None, feature_map_stride=None
):
    batch_size, _, H, W = pred_dict["center"].shape

    batch_center = (
        pred_dict["center"].permute(0, 2, 3, 1).contiguous().view(batch_size, H * W, 2)
    )  # (B, H, W, 2)
    batch_center_z = (
        pred_dict["center_z"]
        .permute(0, 2, 3, 1)
    ...
    batch_vel = (
        pred_dict["vel"].permute(0, 2, 3, 1).contiguous().view(batch_size, H * W, 2)
    ...
    ys, xs = torch.meshgrid(
        [
            torch.arange(0, H, device=batch_center.device, dtype=batch_center.dtype),
